...
@app.get("/production/status", response_model=ProductionStatusReport)
def production_status(
    recent_limit: int = 10,
    include_past: bool = False,
    service: AnalysisService = Depends(get_api_service),
) -> ProductionStatusReport:
    return build_production_status(service, recent_limit=recent_limit, include_past=include_past)
